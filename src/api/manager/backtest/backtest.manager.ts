...
import { GetBacktestHistoryRequest, GetBacktestHistoryResponse, StartBacktestRequest, StartBacktestResponse } from '@manager/backtest/contracts/requestResponse';
...
import { StopBacktestRequest, StopBacktestResponse } from './contracts/requestResponse/stopBacktest';
...
export class BacktestManager implements IBacktestManager {
  ...
  async getBacktestHistory(request: GetBacktestHistoryRequest): Promise<ApiResponse<GetBacktestHistoryResponse>> {
    try {
      const validated = getBacktestHistorySchema.parse(request);
      
      const backtests = await this.backtestService.getBacktestHistory(validated.userId, validated.botId);
